  ...
  stages {
    stage('build') {
      steps {
        ...
        sh "wget http://www.slimjetbrowser.com/chrome/lnx/chrome64_55.0.2883.75.deb && sudo	dpkg -i chrome64_55.0.2883.75.deb && mv /var/jenkins_home/opt/google/chrome/google-chrome /var/jenkins_home/opt/google/chrome/mychrome && echo '#!/usr/bin/env bash' > /google-chrome && echo 'exec /opt/google/chrome/mychrome --no-sandbox --disable-gpu \$@' >> /google-chrome && mv /google-chrome /opt/google/chrome/google-chrome && chmod +x /opt/google/chrome/google-chrome"
      }
    }
